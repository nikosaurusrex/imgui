...
//         - (3) [If using clipper] Calculate index of RangeSrcItem and store in RangeSrcItemIdx. If already using indices in ImGuiSelectionUserData, this is a simple assignment.
...
struct ImGuiMultiSelectIO
{
    ...
    ImGuiSelectionUserData  RangeSrcItem;       //  ms:w  app:r  /               /               // (If using clipper) Begin: Source item (generally the first selected item when multi-selecting, which is used as a reference point).
    int                     RangeSrcItemIdx;    //        app:w  /  ms:r         /               // (If using clipper) Begin: user/app needs to calculate RangeSrcItemIdx from RangeSrcItem. If passing indices to SetNextItemSelectionUserData() they are the same value! Write -1 if you cannot find RangeSrcItem.
    ...
    void Clear()            { memset(this, 0, sizeof(*this)); RequestFocusItem = NavIdItem = RangeSrcItem = RangeFirstItem = RangeLastItem = (ImGuiSelectionUserData)-1; RangeSrcItemIdx = -1; }
};
